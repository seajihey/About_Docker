...
> Spring Boot 기반 애플리케이션의 Docker 이미지를 **753MB → 122MB**로 **84% 경량화**한 실전 가이드

---

## 📋 목차

- [🎯 서론: 왜 다시 '배포'를 이야기하는가?](#-서론-왜-다시-배포를-이야기하는가)
...
- 💸 클라우드 **Egress 비용**의 부담되는 가격
- 🐢 오토스케일링 시 새 인스턴스 기동이 느림

**📌 프로젝트의 목표**: `현상 → 원인 → 해결책 → 비즈니스 가치`의 흐름으로 Docker 이미지 최적화를 완전히 이해합니다.
...
### 3️⃣ 빌드 시점에만 필요한 파일이 이미지에 포함됨

컴파일 도구, 테스트 결과물, 캐시 파일 등은 실행 시점에는 필요 없지만, 빌드 단계에서 생성된 이상 해당 레이어에 포함됩니다.

> 💡 **멀티 스테이지 빌드**를 사용하지 않으면, 이러한 불필요한 빌드 산출물이 그대로 최종 이미지에 남아 이미지가 무거워집니다.
...
최종 운영 단계(Stage 03)의 Dockerfile에 **JVM 옵션을 환경 변수로 설정**하여 컨테이너 환경에 최적화합니다.
...
| 옵션 | 설명 |
|:-----|:-----|
| `-XX:+UseContainerSupport` | JVM이 **cgroups**(Docker 리소스 제한 도구)를 확인하도록 설정. 호스트가 아닌 **컨테이너에 할당된 메모리**를 기준으로 동작 |
| `-XX:MaxRAMPercentage=75.0` | 힙(Heap) 메모리를 컨테이너 가용 자원의 **75%**로 제한 |
...
**👨‍💻 개발자 생산성 향상**: 하루 50번 빌드 × 20초 절약 = **일 15분 절약**
...
큰 이미지: 3,000 × 743MB = 2,176GB → $195.84/월
작은 이미지: 3,000 × 122MB = 357GB  → $32.13/월

월 절감액: $163.71 (연간 $1964.52) 💰
```
...
큰 이미지: 7432MB × 100 = 72.56GB 스토리지
작은 이미지: 122MB × 100 = 11.91GB 스토리지
...
| 시나리오 | 권장 전략 | 이유 |
|:--------:|:---------:|:-----|
| 일반적인 서비스 | 🔧 **Jlink** | 용량 최소 (122MB), 빠른 배포 |
| 서버리스/FaaS | ⚡ **Native Image** | 0.1초 Cold Start |
| 빠른 개발 사이클 | 🏔️ **Alpine + JRE** | 빌드/배포 균형 |
| 레거시 호환성 필요 | 🔀 **Multi-stage** | glibc 호환 유지 |
...
| # | 항목 | 체크 |
|:-:|:-----|:----:|
| 1 | 🏔️ 베이스 이미지를 Alpine 또는 Distroless로 교체했는가? | ⬜ |
| 2 | 🔀 Multi-Stage Build를 적용했는가? | ⬜ |
| 3 | 🔧 Jlink 또는 Native Image를 활용해 런타임을 경량화했는가? | ⬜ |
| 4 | 📋 Dockerfile 명령어를 변경 빈도 순으로 정렬했는가? | ⬜ |
| 5 | 🚫 `.dockerignore`로 불필요한 파일을 제외했는가? | ⬜ |
| 6 | 📦 프로덕션 의존성만 설치했는가? | ⬜ |
